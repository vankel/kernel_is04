...
#if 0
#define LOGI(fmt, args...)      printk(KERN_DEBUG "tsb143f: " fmt, ##args)
#else
#define LOGI(fmt, args...)      do{}while(0)
#endif
#define LOGE(fmt, args...)      printk(KERN_ERR "tsb143f: " fmt, ##args)
...
DEFINE_MUTEX(tsb143f_mtx);
static struct tsb143f_ctrl *tsb143f_ctrl = NULL;
static struct spi_device *tsb143f_spi_dev = NULL;
...
// LED Control
//////////////////////////////////
static void tsb143f_led_control(int ctrl)
{
    switch (ctrl) {
    case LED_OFF:
        if(tsb143f_ctrl->model == TSB_MODEL_NO_0 || tsb143f_ctrl->model == TSB_MODEL_NO_1) {
            pmic_set_led_intensity(LED_LCD, 0);
            pmic_set_led_intensity(LED_KEYPAD, 0);
        } else {
            pmic_secure_mpp_config_i_sink(PM_MPP_13,tsb143f_ctrl->led,
                                          PM_MPP__I_SINK__SWITCH_DIS);
            pmic_secure_mpp_config_i_sink(PM_MPP_18,tsb143f_ctrl->led,
                                          PM_MPP__I_SINK__SWITCH_DIS);
        }
    ...
    case LED_HIGH:
        ...
        if(tsb143f_ctrl->model == TSB_MODEL_NO_0 || tsb143f_ctrl->model == TSB_MODEL_NO_1) {
            ...
            tsb143f_ctrl->led = ctrl == LED_LOW ? PM_MPP__I_SINK__LEVEL_10mA :
...
// SPI SYNC
////////////////////////////////
static int tsb143f_spi(uint8_t *ptx, uint8_t *prx, uint32_t len)
{
    ...
    rc = spi_sync(tsb143f_spi_dev, &msg);
...
// DL
////////////////////////////////
static int tsb143f_dl(struct cfg_dl *dl)
{
    int i;
    uint8_t rx[2];
    ...
    for(i=0;i<4;++i) {
        if (copy_from_user(p[i], dl->dt[i], dl->len[i])) {
            ...
            goto tsb143f_dl_exit1;
        }
        LOGI(" >> %03d.bin DL Start (Size:%d) >>\n", i, dl->len[i]);
        if (tsb143f_spi(p[i], NULL, dl->len[i])) {
        ...
        mdelay(5);
        if (tsb143f_spi(NULL, rx, sizeof(rx))) {
...
// Power ON
////////////////////////////////
static int tsb143f_sensor_poweron(void)
{
...
// Power OFF
////////////////////////////////
static void tsb143f_sensor_poweroff(void)
{
    LOGI("+%s()\n", __func__);
...
// msm_open_control
//---------------------------------------------------------------------
int tsb143f_sensor_init(const struct msm_camera_sensor_info *data)
{
    tsb143f_ctrl = kzalloc(sizeof(struct tsb143f_ctrl), GFP_KERNEL);
    if (!tsb143f_ctrl) {
        LOGE(" -%s kzalloc() Failed!\n",__func__);
        return -ENOMEM;
    }
    ...
    if (data)
        tsb143f_ctrl->sensordata = data;

    tsb143f_ctrl->led = PM_MPP__I_SINK__LEVEL_10mA;
    tsb143f_ctrl->model = tsb_model_get_model_no();

    // Sensor Power ON
    if (tsb143f_sensor_poweron() < 0) {
        kfree(tsb143f_ctrl);
        LOGI("-%s Failed.\n", __func__);
        return -1;
    }
...
// msm_ioctl_control()
//---------------------------------------------------------------------
int tsb143f_sensor_config(void __user *argp)
{
    ...
    mutex_lock(&tsb143f_mtx);
    switch (cfg.cfgtype) {
    case CFG_PWR_UP:
        adxl345_stop_ap();
        rc = tsb143f_dl(&cfg.cfg.dl);
        adxl345_start_ap();
        break;
    ...
    case CFG_SET_LED:
        tsb143f_led_control(cfg.cfg.led);
        break;
...
// msm_release_control()
//---------------------------------------------------------------------
int tsb143f_sensor_release(void)
{
    LOGI("+%s\n", __func__);

    mutex_lock(&tsb143f_mtx);

    tsb143f_led_control(LED_OFF);
    tsb143f_sensor_poweroff();
    kfree(tsb143f_ctrl);
...
/////////////////////////////////////
static int __devinit tsb143f_spi_probe(struct spi_device *spi)
{
    int rc = 0;
...
static int __devexit tsb143f_spi_remove(struct spi_device *spi)
{
    LOGI("+%s\n", __func__);
    return 0;
}

static struct spi_driver tsb143f_spi_driver = {
    .driver     = {
        .name   = "tsb143f",
        .owner  = THIS_MODULE,
    },
    .probe      = tsb143f_spi_probe,
    .remove     = __devexit_p(tsb143f_spi_remove),
};

static int tsb143f_sensor_probe(const struct msm_camera_sensor_info *info,
                                struct msm_sensor_ctrl *s)
{
    s->s_init = tsb143f_sensor_init;
    s->s_release = tsb143f_sensor_release;
    s->s_config  = tsb143f_sensor_config;

    return spi_register_driver(&tsb143f_spi_driver);
}

static int __tsb143f_probe(struct platform_device *pdev)
{
    return msm_camera_drv_start(pdev, tsb143f_sensor_probe);
}

static struct platform_driver msm_camera_driver = {
    .probe = __tsb143f_probe,
    .driver = {
...
static int __init tsb143f_init(void)
{
    return platform_driver_register(&msm_camera_driver);
}

module_init(tsb143f_init);
