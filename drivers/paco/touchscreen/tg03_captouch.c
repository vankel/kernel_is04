...
typedef struct  {
    ...
    struct work_struct tg03cap_wq_calibration_func;
...
static int get_tch_and_atch ( tg03_ts_data *ts, uint8_t *data )
{
    uint8_t     val ;
    ...
    val = 0xF3;
    tg03cap_i2c_write( ts, &val, get_object_address(GEN_COMMANDPROCESSOR_T6)+5, 1 );

    mdelay(10);

    for ( cnt = 0 ; cnt < MAX_INIT_WAIT ; cnt += 2 ){
        tg03cap_i2c_read( ts, read_buff, get_object_address(DEBUG_DIAGNOSTIC_T37), 2 );
    ...
    tg03cap_i2c_read( ts, read_buff, get_object_address(DEBUG_DIAGNOSTIC_T37), 82 );
...
static int check_tch_and_atch( tg03_ts_data *ts )
{
    int         retval = 0;
    uint8_t     tmp_tch[128];
    ...
    for ( cnt = 0 ; cnt < tg03cap_info.matrix_x_size*2 ; cnt += 2 ){
...
static int tg03cap_get_device_id( tg03_ts_data *ts, uint8_t *read_buff )
{
    ...
    model_no = dev_model_get_model_no();

    if ( model_no <= DEV_MODEL_NO_3 ){
    ...
    switch ( model_no ){
        ...
        case DEV_MODEL_NO_3:
            retval = MODEL_NO_PR_WTK;
            break;
        default:    /* DEV_MODEL_NO_PR and later */
...
static void calibration_timeout_func( unsigned long ptr )
{
    ...
    queue_work( ts->tg03cap_wq, &ts->tg03cap_wq_calibration_func );

}

static void calibration_work_func( struct work_struct *work )
{
    tg03_ts_data *ts = container_of( work, tg03_ts_data, tg03cap_wq_calibration_func );
    char    val = 0x00;
    ...
    switch(cmd){
        case IOCTL_SET_DBGMODE:
            if ( ioctl_irq_sts == 0 ){
                ...
                retval = request_irq( ts->irq, tg03cap_interrupt, IRQF_TRIGGER_LOW, "touchscreen", ts );
                ioctl_irq_sts = 0; 
            }
            break;
...
static int __devinit tg03cap_pdev_probe(struct platform_device *pdev)
{
    ...
    INIT_WORK( &ts->tg03cap_wq_calibration_func, calibration_work_func );

    /* irq init */
    ts->irq = MSM_GPIO_TO_INT( ts->gpio_int );
    result = request_irq(ts->irq, tg03cap_interrupt, IRQF_TRIGGER_LOW, "touchscreen", ts);
    if (result){
